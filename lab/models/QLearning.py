...
class QLearning:
    ...
    def move(self, learn=True):
        ...
        if not random_choice:
            current_state_Qs = np.array(
                [
                    self.Q[self.position][current_state_action]
                    for current_state_action in possible_actions
                ]
            )
        ...
        next_state_Qs = np.array(
            [
                self.Q[action][next_state_action]
                for next_state_action in next_state_all_actions
            ]
        )
    ...
    def set_init_position(self):
        print(f"Initialization to random position - gen -> {self.generation}")
        ...
        for y in range(size):
            for x in range(size):
                ...
                for _y, _x in movement:
                    edge_y = y + _y
                    edge_x = x + _x
                    in_env = (edge_y >= 0 and edge_y < size) and (
                        edge_x >= 0 and edge_x < size
                    )
                    if not in_env:
                        continue
