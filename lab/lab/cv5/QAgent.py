...
class QAgent:
    ...
    def l_r(self, epoch):
        l = (
            LEARNING_RATE
            if CONSTANT_LR
            else max(LEARNING_RATE, min(0.5, 1.0 - math.log10((epoch + 1) / 25)))
        )
        return l
    ...
    def map_state(self, env, state):
        ...
        for i in range(len(state)):
            ...
            scaling = (current_state + abs(current_lower_b)) / (
                current_upper_b - current_lower_b
            )
            new_state = int(round((current_bucket - 1) * scaling))
            new_state = min(
                current_bucket - 1, max(0, new_state)
            )  # bounderies 0 - bucket size
    ...
    def update_Q(self, state, action, next_state, reward):
        ...
        update = self.learning_rate * (
            DISCOUNT_FACTOR * max_value + reward - self.Q[tuple(state)][action]
        )
